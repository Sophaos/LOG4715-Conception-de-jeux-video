...
namespace UnityStandardAssets.Cameras
{
    ...
    public class AutoCam : PivotBasedCameraRig
    {
        ...
        [SerializeField] private PlatformerCharacter2D m_character2D;       // the 2d character
        ...
        protected override void FollowTarget(float deltaTime)
        {
            ...
            Vector3 t_position = m_Target.position;        // keep a tracker for the position
            if (!m_character2D.getGrounded())              // if the player is not grounded
            {
                t_position.y = transform.position.y;       // track the y position (if not the y pos stands still)
            }

            // camera position moves towards target position:
            transform.position = Vector3.Lerp(transform.position, t_position, deltaTime*m_MoveSpeed);
